...
class ProjectTestCase(TestCase):
    ...
    def test_project_estimate(self):
        project = Project.objects.create(
            client=self.client, name="Testing", estimate=5000.00
        )
        task = Task.objects.create(name="Testing", hourly_rate=100.00)
...
class EntryTestCase(TestCase):
    ...
    def test_entry_created(self):
        ...
        entries = Entry.objects.filter(duration__lte=timedelta(hours=1, minutes=30))
...
class ReportsTestCase(TestCase):
    ...
    def test_export_response(self):
        ...
        self.assertEqual(
            report.get("Content-Disposition"), 'attachment; filename="report.csv"'
        )
    ...
    def test_export_param_client(self):
        self.assert_entries_by_params({"project__client": Client.objects.first().id})
