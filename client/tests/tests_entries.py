...
class EntriesTestCase(SeleniumTestCase):
    ...
    def test_entries_change(self):
        ...
        cleaned_date = datetime.strptime(
            entry_modal_date.get_attribute("value"), "%Y-%m-%d"
        ).date()
        cleaned_project = int(entry_modal_project.get_attribute("value").split("/")[-2])
        cleaned_task = int(entry_modal_task.get_attribute("value").split("/")[-2])
        cleaned_duration = parse_duration(entry_modal_duration.get_attribute("value"))
