...
gulp.task(
  ...
  gulp.series("build:webpack:production", () => {
    ...
    return spawn("pipenv", command, { stdio: "inherit", env: process.env });
  })
);

gulp.task("manage:runserver", () => {
  return spawn("pipenv", ["run", "python", "manage.py", "runserver"], {
    stdio: "inherit"
  });
});

gulp.task("manage:makemigrations", () => {
  return spawn("pipenv", ["run", "python", "manage.py", "makemigrations"], {
    stdio: "inherit"
  });
});

gulp.task("manage:migrate", () => {
  return spawn("pipenv", ["run", "python", "manage.py", "migrate"], {
    stdio: "inherit"
  });
});

gulp.task("manage:createsuperuser", () => {
  return spawn("pipenv", ["run", "python", "manage.py", "createsuperuser"], {
    stdio: "inherit"
  });
});

gulp.task("manage:fake", () => {
  return spawn("pipenv", ["run", "python", "manage.py", "fake"], {
    stdio: "inherit"
  });
});
